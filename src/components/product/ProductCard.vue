<template>
  <div class="product-card bg-white rounded-2xl shadow-lg overflow-hidden group hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2" :style="{ animationDelay: `${index * 100}ms` }">
    <!-- Image Container -->
    <div class="relative h-48 overflow-hidden">
      ...
      <!-- Favorite Badge -->
      <div v-if="product.is_favorite" class="absolute top-3 left-3">
      ...
      <div class="absolute top-3 right-3">
        <span class="inline-block px-2.5 py-1 bg-white/90 backdrop-blur-sm text-brown-700 text-xs font-semibold rounded-full shadow-sm">
          {{ product.category }}
        </span>
      </div>
    </div>
    ...
    <div class="p-4">
      ...
      <!-- Description -->
      <p v-if="product.description" class="text-xs text-gray-500 mb-3 line-clamp-2">
        {{ product.description }}
      </p>

      <!-- Price Options - Same as MenuItem -->
      <div class="flex flex-col gap-2">
        <!-- Hot Price Option -->
        <div v-if="product.price_hot" class="flex justify-between items-center p-2 rounded-lg bg-orange-50 hover:bg-orange-100 transition-colors">
          <div class="flex items-center gap-2">
            <span class="text-lg">🔥</span>
            <div>
              <span class="text-xs text-gray-500">Hot</span>
              <p class="font-bold text-gray-900">{{ product.price_hot }}K</p>
            </div>
          </div>
          <button
        ...
        <!-- Cold Price Option -->
        <div v-if="product.price_cold" class="flex justify-between items-center p-2 rounded-lg bg-blue-50 hover:bg-blue-100 transition-colors">
          <div class="flex items-center gap-2">
            <span class="text-lg">❄️</span>
            <div>
              <span class="text-xs text-gray-500">Cold</span>
              <p class="font-bold text-gray-900">{{ product.price_cold }}K</p>
            </div>
          </div>
          <button
